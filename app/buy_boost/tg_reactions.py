...
@router.callback_query(k.Menu_callback.filter(F.menu == 'tg_reactions'))
async def menu(call: CallbackQuery, callback_data: k.Menu_callback, state: FSMContext):
    ...
    photo = FSInputFile('photos/reactions.jpg')
    kb = k.tg_reactions_kb()
    await call.message.answer_photo(photo=photo, reply_markup=kb)



@router.callback_query(k.Buy_info.filter(F.menu == 'tg_reactions'))
